...
func TestDemoFixtureIsVersionedAndDeterministic(t *testing.T) {
	...
	if got, want := first.DataCounts(), (Counts{Instruments: 3, DailyBars: 6, FinancialMetrics: 6}); got != want {
		t.Fatalf("fixture counts = %#v, want %#v", got, want)
	}
	if first.Instruments[0] != second.Instruments[0] || first.DailyBars[0] != second.DailyBars[0] || first.FinancialMetrics[0] != second.FinancialMetrics[0] {
